...
sprintf(str, "/sys/class/gpio/gpio%d/value", gpio);
   ...
   if ((interruptfd = open(str, O_RDONLY)) < 0)
   {
   ...
   lseek(interruptfd, 0, SEEK_SET);    /* consume any prior interrupt */
   read(interruptfd, buf, sizeof buf);

   poll(&pfd, 1, -1);         /* wait for interrupt */

   lseek(interruptfd, 0, SEEK_SET);    /* consume interrupt */
   read(interruptfd, buf, sizeof buf);
